...
{
    public class HanjiSpotTrader : IHanjiTrader
    {
        ...
        private readonly HanjiSpot _onchainClobSpot;
        private readonly RpcClient _rpc;
        private readonly HanjiGasLimits? _defaultGasLimits;
        ...
        public HanjiSpotTrader(
            ...
            HanjiSpot onchainClobSpot,
            RpcClient rpc,
            HanjiGasLimits? defaultGasLimits = null,
            ILogger<HanjiSpotTrader>? logger = null)
        {
            ...
            _onchainClobSpot = onchainClobSpot ?? throw new ArgumentNullException(nameof(onchainClobSpot));
            _onchainClobSpot.Executor.TxMempooled += Executor_TxMempooled;
            _onchainClobSpot.Executor.TxFailed += Executor_TxFailed;
            _onchainClobSpot.Executor.Error += Executor_Error;
        ...
        {
            var fromAddress = _onchainClobSpot.Executor.Signer.GetAddress();
            ...
            var placeOrderRequestId = await _onchainClobSpot.PlaceOrderAsync(new PlaceOrderParams
            {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var claimOrderRequestId = await _onchainClobSpot.ClaimOrderAsync(new ClaimOrderParams
            {
                OrderId = orderId,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var changeOrderRequestId = await _onchainClobSpot.ChangeOrderAsync(new ChangeOrderParams
            {
                OldOrderId = orderId,
        ...
        public async Task<bool> PendingOrderCancelAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var isCanceled = await _onchainClobSpot.Executor.TryCancelRequestAsync(
                placeOrderRequestId,
                cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            foreach (var request in requests.OfType<HanjiCancelPendingOrderRequest>())
            ...
            requests = requests
                .Where(r => r is not HanjiCancelPendingOrderRequest)
            ...
            foreach (var (batchGasLimit, batchRequests) in SplitIntoSeveralBatches(requests))
            {
                ...
                var batchChangeOrderRequestId = await _onchainClobSpot.BatchChangeOrderAsync(new BatchChangeOrderParams
                {
                    OrderIds = orderIds,
                    Prices = prices,
                    Quantities = qtys,
                ...
                {
                    if (request is HanjiPlaceOrderRequest placeOrderRequest)
                    {
                        var pendingOrder = new Order(
                    ...
                    else if (request is HanjiChangeOrderRequest changeOrderRequest)
                    {
                    ...
                    else if (request is HanjiClaimOrderRequest claimOrderRequest)
                    {
        ...
        private void HanjiClient_StateStatusChanged(object? sender, StateStatus status)
        {
            ...
            _webSocketClient.SubscribeUserOrdersChannel(
                userAddress: _onchainClobSpot.Executor.Signer.GetAddress(),
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (args.IsSnapshot)
            {
                ...
                var (activeOrders, error) = await _restApi.GetActiveOrdersAsync(
                    _onchainClobSpot.Executor.Signer.GetAddress(),
        ...
        private bool TryNormalizePrice(decimal price, out ulong hanjiPrice)
        {
            hanjiPrice = price.ToHanjiPrice(_symbolConfig.PricePrecision, out var rest);

            return rest == 0;
        }

        private bool TryNormalizeQty(decimal qty, out BigInteger hanjiQty)
        {
            var multiplier = BigInteger.Pow(10, _symbolConfig.TokenX.Decimals - _symbolConfig.ScallingFactorX);

            hanjiQty = qty.Multiply(multiplier);

            return hanjiQty.Divide(multiplier) == qty;
        }

        private IEnumerable<(ulong?, IEnumerable<IHanjiRequest>)> SplitIntoSeveralBatches(IEnumerable<IHanjiRequest> requests)
        {
            ...
            var batch = new List<IHanjiRequest>();

            foreach (var request in requests)
            {
                var gasLimit = request switch
                {
                    HanjiPlaceOrderRequest => _defaultGasLimits.PlaceOrder,
                    HanjiClaimOrderRequest => _defaultGasLimits.ClaimOrder,
                    HanjiChangeOrderRequest => _defaultGasLimits.ChangeOrder,
                    _ => 0ul
                };
